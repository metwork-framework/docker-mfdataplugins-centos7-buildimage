...
ARG CACHEBUST=0
RUN yum clean all
RUN yum --disablerepo=* --enablerepo=metwork_${BRANCH} -q list metwork-mfdata-full* 2>/dev/null |sort |md5sum |awk '{print $1;}' > /tmp/yum_cache
...
RUN yum clean all
RUN yum -y install metwork-mfdata-full initscripts cronie vim
